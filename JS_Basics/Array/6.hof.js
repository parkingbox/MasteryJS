...
const product = [item1, item2, item3, item3];
let found = product.find((value) => value.name === '🍪');
console.log(found);

// findIndex : 제일 먼저 조건에 맞는 아이템의 인덱스를 반환
result = product.findIndex((value) => value.name === '🍪');
console.log(result);

// 배열의 아이템들이 부분적으로 조건(콜백함수)에 맞는지 확인 some
result = product.some((item) => item.name ==='🍪');
console.log(result);

// 배열의 아이템들이 모두 조건(콜백함수)에 맞는지 확인 every
